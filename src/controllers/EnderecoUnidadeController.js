...
import EnderecoUnidadeValidacao from "../services/EnderecoUnidadeValidacao.js"
...
class EnderecoUnidadeController {
	...
	static rotas(app) {
		...
		app.get("/enderecoUnidade/:id", async (req, res) => {
			...
			const valido = await EnderecoUnidadeValidacao.validarBusca(id)
			if (valido) {
		...
		app.post("/enderecoUnidade", async (req, res) => {
			const body = req.body
			const valido = EnderecoUnidadeValidacao.validarCampos(...Object.values(body))
			if (valido) {
